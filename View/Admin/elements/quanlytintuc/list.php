    ...
    <script>
           const getNewss = async()=>{
                ...
                try {
                    ...
                    console.log(res);
                    newss.forEach((news, index)=>{
                        const tr = document.createElement('tr')
                        tr.innerHTML = `
                            <td>${index+1}</td>
                            <td>${news.name}</td>
                            <td>${news.descriptions}</td>
                            <td> <img src="${news.image}" id="img" alt="anhloi"></td>  
                            <td>ok oko ok</td>
                            <td>
                                <button class="btn btn-success" ><a href="edit.html?id=${news.id}">Update</a></button>
                                <button class="btn btn-danger" onclick="Deletecourses(${news.id})">Delete</button>
                            </td>
                        `
                        tbody.append(tr)
                    })
            ...
            const Deletenews= async (id)=>{
                if(confirm('Bạn có chắc muốn xóa chứ???')){
                    try {
                       const res = await fetch(`http://localhost/Duan1_nhom7/DuAn1/Api/${id}`, {
                        method: 'DELETE',
                       })
                    if(res.ok){
